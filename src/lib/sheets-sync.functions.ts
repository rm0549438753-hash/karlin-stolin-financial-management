...
export const syncFromGoogleSheet = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    async function ensureSubcat(catName: string | null, subName: string): Promise<string | null> {
      ...
      subMap.set(`${cn}||${normName(created.name)}`, created.id);
      subMap.set(`||${normName(created.name)}`, created.id);
      return created.id;
    }
    ...
    for (const s of sheets) {
      ...
      for (const r of inserts) {
        ...
        const subId = r._subcategory_name ? await ensureSubcat(r._category_name ?? null, r._subcategory_name) : null;
